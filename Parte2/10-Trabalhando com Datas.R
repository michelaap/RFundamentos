...
as.Date("2020-08-05")
as.Date("20200805") # Erro
as.Date("Aug-05-20", format = "%b-%d-%y")
as.Date("05 Aug, 2020", format = "%d %B, %Y")
...
date1 <- "Aug 05, '20 hours:20 minutes:53 seconds:45"
date1_convert <- as.POSIXct(date1, format = "%B %d, '%y hours:%H minutes:%M seconds:%S")
date1_convert


# Operações com Datas
data_de_hoje <- as.Date("2020-08-05", format = "%Y-%m-%d")
data_de_hoje
data_de_hoje + 1

my_time <- as.POSIXct("2020-07-05 20:54:134")
my_time
my_time + 1
...
partida <- ymd_hms("2020-08-01 14:00:00", tz = "Pacific/Auckland")
